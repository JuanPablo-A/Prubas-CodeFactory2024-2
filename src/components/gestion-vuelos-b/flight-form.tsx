...
export const FlightForm = (props: FlightFormProps) =>{
  ...
  const [flightNumber, setFlightNumber] = useState(props.flight?.flightNumber ?? "");
  const [airplaneTypeId, setAirplaneTypeId] = useState(props.flight?.airplaneType.id ?? "");
  const [flightTypeId, setFlightTypeId] = useState<"1" | "2">((props.flight?.flightType.id as "1" | "2") ?? "1");
  const [price, setPrice] = useState(props.flight?.price ?? 0);
  const [surchargePercentage, setSurchargePercentage] = useState(props.flight?.surchargePercentage ?? 0);
  const [taxPercentage, setTaxPercentage] = useState(props.flight?.taxPercentage ?? 0);
  
  const [departureDate, setDepartureDate] = useState(props.flight ? new Date(`${props.flight?.departureDate} ${props.flight?.departureTime}`).toISOString().slice(0, 16) : "");
  const [arrivalDate, setArrivalDate] = useState(props.flight ? new Date(`${props.flight?.arrivalDate} ${props.flight?.arrivalTime}`).toISOString().slice(0, 16) : "");
  const [originIata, setOriginIata] = useState(props.flight?.origin.iataCode ?? "");
  const [destinationIata, setDestinationIata] = useState(props.flight?.destination.iataCode ?? "");
  ...
  return <form className="w-full p-4 pt-0 sm:p-12 sm:pt-0 flex flex-col gap-8">
